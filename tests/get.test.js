...
describe('getAll function', () => {
  ...
  it('should return a list of websites and status 200 for a valid request with a valid token', async () => {
    ...
    const website2 = {
      __v: 0, _id: '6489b3160eb569aee1edaab7', backups: [], cpu: 4, description: 'good!!!!', domain: ['mywebsite.com', 'www.mywebsite.com'], memory: 8, status: '200', title: 'My Website2', typeOfDomain: 'Primary',
    };
    ...
    expect(res.send).toHaveBeenCalledWith([website1, website2]);
  });
  it('should return status 404 and an error message for an invalid request', async () => {
    const token = 'valid_token';
    const errorMessage = 'Error message';
    req.headers.authorization = `Bearer ${token}`;
    websiteModel.find = jest.fn().mockRejectedValue(new Error(errorMessage));
    await controller.getAll(req, res);
    expect(websiteModel.find).toHaveBeenCalledTimes(1);
